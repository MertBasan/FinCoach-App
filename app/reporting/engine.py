...
def _is_cost_of_sales(account: Account | None) -> bool:
    """COGS heuristic: any expense-typed account whose code starts with '5'.
    Matches the default UK SME CoA (5000-series = Cost of Sales)."""
    if not account or account.type != "expense":
        return False
    return account.code.startswith("5")

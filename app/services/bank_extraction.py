"""
FinCoach bank-statement extractor (library form).

Adapted from the original Streamlit app — UI removed, all parsers kept:
Halkbank (V1+V2), Akbank, Ziraat (vector + OCR), Yapı Kredi,
Kuveyt Türk, Türkiye Finans Katılım, generic fallback.

Public entry points:
  process_pdf(pdf_bytes, filename) -> StatementResult
  result_to_dataframe(result) -> pd.DataFrame
  df_to_xlsx_bytes(df) -> bytes
  build_output_basename(result) -> str
  OCR_AVAILABLE: bool
"""
